...
docs.DocManager = {
	...
	onResize: function() {
		...
		this.content
			.height(windowHeight  - footerHeight+ (docs.Features.hasTouch ? 50 : 0))
		...
		t.audioBtn.on('click', function() {
			if (t.audioWindow.content.is(':visible')) {
				...
				if (sectionId != t.currentAudio) {
					...
					t.player.pause();
					t.player.setSrc(CFG.content+'audio/' + t.selector.val() + '/' + filename);
					t.player.load();
	...
	*/
	...
	t.infoBtn.on('click', function(e) {
		...
		if (t.about.is(':visible')) {
			...
			$.ajax({
				url: CFG.content+'bibles/' + t.selector.val() + '/about.html',
				dataType: 'html',
...
docs.Document.prototype = {
	...
	updateVersion: function(version) {
		...
		if (styleLink.length == 0) {
			styleLink = $('<link id="' + styleId + '" rel="stylesheet" href="'+CFG.content+'bibles/' + version + '/style.css" />')
							.appendTo($('head'));
		}
		...
		// find audio
		$.ajax({
			url:CFG.content+'audio/' + version + '/version.json',
	...
	load: function(fragmentId, action) {
		...
		var t = this,
			...
			url = CFG.content+'bibles/' + this.selector.val() + '/';
						...
						t.selector.find('option').each(function(index, opt) {
							...
							if ($(opt).attr('value') === switchToVersion) {
								t.selector[0].selectedIndex = index;
					        		...
					        		// find and load styles
						//yb
						var styleId = 'style-' + switchToVersion,
						styleLink = $('#' + styleId);
			
						if (styleLink.length == 0) {
							styleLink = $('<link id="' + styleId + '" rel="stylesheet" href="'+CFG.content+'bibles/' + switchToVersion + '/style.css" />')
							.appendTo($('head'));
						}
						//end yb	
