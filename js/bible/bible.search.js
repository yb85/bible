...
bible.BibleSearch = {
	...
	baseBiblePath: CFG.content+'bibles/',
	
	baseLemmaPath: CFG.content+'lexicons/index/',
	...
	search: function(text, version, chapterCallback, endedCallback) {
		...
		$.ajax({
			dataType: 'json',
			async: false,
			url: s.baseBiblePath + s.version + '/version.json',
success: function(data){console.log("success !!!");s.bookRange=data["books"]},
error: function(){console.log("error !!!");s.bookRange = bible.DEFAULT_BIBLE}});
console.log(s.version);

console.log(s.bookRange);

s.bookOsisID = s.bookRange[0];

		// SETUP REGEX For next word
		s.isLemmaSearch = /(G|H)\d{1,5}(v|n|\+)?/.test(s.searchText);
		
		/*if (s.isLemmaSearch) {
			switch(String(s.searchText.slice(-1))){
				case '+':
					s.bookRange = bible.DEFAULT_BIBLE;
					s.searchText=s.searchText.slice(0,-1);
					console.log("lsearch>+");break;
				case 'v':
					s.bookRange = bible.OT_BOOKS;
					s.searchText=s.searchText.slice(0,-1);					
					console.log("lsearch>v");break;					
				case 'n':
					s.bookRange = bible.NT_BOOKS;
					s.searchText=s.searchText.slice(0,-1);					
					console.log("lsearch>n");break;					
				default :
					console.log("lsearch>def");										
			...
			for (i=0, il=verses.length; i<il; i++) {
				...
				for (var j=0, jl=s.searchRegExp.length; j<jl; j++) {
					...
					if (s.isLemmaSearch) {
						...
						if($(verseSpan).text().match(s.searchRegExp[j]) != null){
