...
bible.versions = {
	...
	loadVersionManifest: function() {
		...
		$.ajax({
			url: CFG.content+'bibles/versions.json',
			dataType: 'json',
			cache: false,
			done: function(x) {
			...
			success: function(data) {
				...
				console.log(t.allVersions);
				t.loadNextVersion();
			},
			error: function(x) {
				//if(x.status==0) successCallback(null, errorText, x);
				console.log("error :: "+x);
				t.setContentDir(); 				
			}
		});
	 ...
	 setContentDir : async () => {
		...
		chooser.id='chooserinput';
		chooser.setAttribute('type', 'file');
		chooser.setAttribute('nwdirectory', 'true');
		chooser.setAttribute('nwdirectorydesc', 'Please locate your Bible Library :');
		await delay(200);
		chooser.addEventListener('change', function(evt) {
			CFG.content=encodeURI("file://"+this.value+"/");
	...
	loadNextVersion: function() {
		...
		if (t.loadingVersionIndex < t.allVersions.length) {	
			...
			$.ajax({
				url: CFG.content+'bibles/' + versionFolder + '/version.json',
				dataType: 'json',
				success: function(data) {
